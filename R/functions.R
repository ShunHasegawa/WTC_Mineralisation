...
PltMean <- function(data){
  ylabs <- c(expression(atop("Nitrification rates", paste((mg~dry_soil_mg^-1~day^-1)))),
             expression(atop("N mineralisation rates", paste((mg~dry_soil_mg^-1~day^-1)))),
             expression(atop("P mineralisation rates", paste((mg~dry_soil_mg^-1~day^-1)))))
  
  ylab <- ifelse(length(unique(data$variable)) > 1, expression((mg~dry_soil_mg^-1~day^-1)),
